...
public class SetUpDB {
    ...
    private static void setUpTableLogData(Connection connection) {
        final String SQL = "CREATE TABLE IF NOT EXISTS Log (" +
                " date TEXT NOT NULL,"+
    ...
    private static void setUpLogData() {
        try {
            LogDataDao dao = DaoFactory.getDaoFactory().createLogDataDao();
            dao.create(new LogData(convertStringToLocalDate("2024-04-30"), "Log wurde erstellt!"));
        } catch (SQLException exception) {
            exception.printStackTrace();
        }
    }
